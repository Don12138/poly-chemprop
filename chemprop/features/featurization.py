...
from chemprop.rdkit import make_mol, make_polymer_mol,make_normal_mol
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        if type(mol) == str:
            if self.is_reaction:
                ...
                if mtype == 0:
                    fragment_weights = [0.5,0.5]
                elif mtype == 1:
                    fragment_weights = [0.75,0.25]
                else:
                    fragment_weights = [0.25,0.75]
                mol = make_normal_mol(mol, self.is_explicit_h, self.is_adding_hs,fragment_weights)
        ...
        if not self.is_reaction and not self.is_polymer:
            ...
            self.w_atoms = [atom.GetDoubleProp('w_frag') for atom in mol.GetAtoms()]
        ...
        edge_index = [[],[]]
        for i in range(len(self.b2a)):
            edge_index[0].append(self.b2a[i])
            edge_index[1].append(self.b2a[self.b2revb[i]])

        self.distance, self.node_path, self.edge_path = get_distance_and_path(Data(
        x=torch.tensor(self.w_atoms),
        edge_index=torch.tensor(edge_index),),max_len=69,max_distance=100) #it should be args.max_atom_len but I feel tired to do it 
    ...
    * :code:`a2a`: (Optional): A mapping from an atom index to neighboring atom indices.
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        ...
        for mol_graph in mol_graphs:
            ...
            self.ctype.append(mol_graph.ctype)
            ...
            self.distances.append(mol_graph.distance)
            self.node_paths.append(mol_graph.node_path)
            self.edge_paths.append(mol_graph.edge_path)
        ...
        self.ctype = torch.LongTensor(self.ctype)
        self.distances = torch.stack(self.distances,dim=0)
        # 下面是不要的
        # self.distances = np.array(self.distances)
        # self.distances = torch.LongTensor(self.distances)   # 一定为long吗
        # 下面是转化为np数组
        # self.f_atoms = np.array(f_atoms, dtype=np.float32)
        # self.f_bonds = np.array(f_bonds, dtype=np.float32)
        # self.w_atoms = np.array(w_atoms, dtype=np.float32)
        # self.w_bonds = np.array(w_bonds, dtype=np.float32)
        # self.a_scope = np.array(self.a_scope, dtype=np.int64)
        # self.b_scope = np.array(self.b_scope, dtype=np.int64)
        # self.a2b = np.array([a2b[a] + [0] * (self.max_num_bonds - len(a2b[a])) for a in range(self.n_atoms)], dtype=np.int64)
        # self.b2a = np.array(b2a, dtype=np.int64)
        # self.b2revb = np.array(b2revb, dtype=np.int64)
        # self.degree_of_polym = np.array(self.degree_of_polym, dtype=np.int64)
        # self.ctype = np.array(self.ctype, dtype=np.int64)
    ...
    def get_components(self, atom_messages: bool = False):
        ...
        * :code:`b_scope`
        ...
        #     "f_bonds": self.f_bonds,
        #     "w_atoms": self.w_atoms,
        #     "w_bonds": self.w_bonds,
        #     "a2b": self.a2b,
        #     "b2a": self.b2a,
        #     "b2revb": self.b2revb,
        #     "a_scope": self.a_scope,
        #     "b_scope": self.b_scope,
        #     "degree_of_polym": self.degree_of_polym,
        #     "graph_native_info": self.graph_native_info,
        #     "ctype": self.ctype
        # }
        return self.f_atoms, f_bonds, self.w_atoms, self.w_bonds, self.a2b, self.b2a, self.b2revb, \
               self.a_scope, self.b_scope, self.degree_of_polym,self.distances, self.node_paths, self.edge_paths, self.ctype 
...
def mol2graph(mols: Union[List[str], List[Chem.Mol], List[Tuple[Chem.Mol, Chem.Mol]]],
              ctypes:List[int],mtypes:List[int],
    ...
    """
    return BatchMolGraph([MolGraph(mol,ctype, mtype, af, bf, 
                          ...
                          for mol, af, bf, ctype, mtype
                          in zip_longest(mols, atom_features_batch, bond_features_batch,ctypes,mtypes)])
