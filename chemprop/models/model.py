...
class MoleculeModel(nn.Module):
    ...
    def __init__(self, args: TrainArgs):
        ...
        # self.ffn[-1].bias.data = torch.tensor([float(x) for x in args.data_mean],requires_grad=True)
    ...
    def create_ffn(self, args: TrainArgs) -> None:
        ...
        if args.ffn_num_layers == 1:
            ffn = [
                dropout,
                nn.Linear(first_linear_dim+1, self.output_size)
            ]
        else:
            ffn = [
                dropout,
    ...
    def forward(self,
        ...
        for i in range(len(self.ffn)):
            if i == len(self.ffn)-1:
                output = torch.cat((output,batch[0].ctype.unsqueeze(1).to("cuda")), dim=1)
            output = self.ffn[i](output)
